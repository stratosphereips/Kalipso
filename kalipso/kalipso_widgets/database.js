...
class Redis{
    ...
    getCollectionMembers(key){
        return new Promise((resolve, reject) => {
            this.db.type(key, (typeErr, keyType) => {
                if (typeErr) {
                    reject(typeErr)
                    return
                }
                ...
                if (keyType === "set") {
                    this.db.smembers(key, (err, reply) => {
                        if (err) {
                            reject(err)
                            return
                        }
                ...
                reject(new Error(`Unsupported Redis type for ${key}: ${keyType}`))
            })
        })
    }

    getAllProfiles(){
        return this.getCollectionMembers("profiles").catch(err => {
            console.log("Error in getAllProfiles() in database.js to retrieve all profiles from the database. Error: ", err);
            throw err
        })
    }
    ...
    /*Get host IP*/
    getHostIP(){
	    return this.getCollectionMembers('hostIP').catch(err => {
	      console.log(err);
	      throw err
	  	})
    }
